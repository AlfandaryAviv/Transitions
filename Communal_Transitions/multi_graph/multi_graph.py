...
class MultiGraph:
    ...
    def community_graph(self, cross_year_factor=1, neighbours_factor=1):
        # build nodes
        community_gnx = nx.DiGraph() if self._directed else nx.Graph()
        blue_edges = {}
        for edge in list(self._gnx_multi.edges(data=True)):
            # collect time stamps for each node
            idx_years = [self.name_to_index(graph_name) for graph_name in edge[2]]
            src_time_stamps, dst_time_stamps = blue_edges.get(edge[0], set()), blue_edges.get(edge[1], set())
            blue_edges[edge[0]] = src_time_stamps | set(idx_years)
            blue_edges[edge[1]] = dst_time_stamps | set(idx_years)

            # build edges in times (red edges)
            for graph_name in edge[2]:
                idx_year = "_" + str(self.name_to_index(graph_name))
                # community_gnx.add_edge(str(edge[0]) + idx_year, str(edge[1]) + idx_year, weight=float(edge[2][graph_name]))
                community_gnx.add_edge(str(edge[0]) + idx_year, str(edge[1]) + idx_year, weight=float(neighbours_factor))

        # build edges between times (blue edges)
        total_blue_edges = 0
        years_interval = len(self._list_id)
        for node, time_stamps in blue_edges.items():
            sorted_years = sorted(list(time_stamps))
            total_blue_edges += len(time_stamps) - 1
            for prev_year, next_year in zip(sorted_years[:-1], sorted_years[1:]):
                # community_gnx.add_edge(node + "_" + str(prev_year), node + "_" + str(next_year),
                #                        weight=float((years_interval - (next_year - prev_year)) / years_interval * (cross_year_factor)))
                community_gnx.add_edge(node + "_" + str(prev_year), node + "_" + str(next_year),
                                               weight=float((cross_year_factor)**(next_year - prev_year)))
        #         community_gnx.add_edge(node + "_" + str(prev_year), node + "_" + str(next_year),
        #                                weight=float( ((years_interval)/(next_year - prev_year))*(cross_year_factor) ))
        return community_gnx, total_blue_edges
